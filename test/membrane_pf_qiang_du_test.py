...
M1 = 500  # constraint parameter for volume
M2 = 50  # constraint parameter for surface area
...
vol = -0.3
suf = 0.3
# Weak formulation of c
F0 = inner(c, q) * dx - inner(c0, q) * dx - dt * eps1 * inner(grad(mu_mid), grad(q)) * dx \
     - dt * 1 / eps1 * inner(mu_mid * (3 * c_mid**2 - 1), q) * dx
# Weak formulation of mu
F1 = inner(mu + 1 / eps1**2 * (c**2 - 1) * c, v) * dx + inner(grad(c), grad(v)) * dx
# Constaint of total volume
F2 = dt * M1 * inner((assemble_scalar(form(c_mid * dx)) - vol), q) * dx

BB = assemble_scalar(form(eps1 / 2 * inner(grad(c_mid), grad(c_mid)) * dx \
     + 1 / 4 / eps1 * (c_mid**2 - 1)**2 * dx)) - suf

F3 = dt * M2 * inner(BB / eps1 * (c_mid**2 - 1) * c_mid, q) * dx \
     + dt * M2 * BB * eps1 * inner(grad(c_mid), grad(q)) * dx
